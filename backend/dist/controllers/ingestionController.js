...
exports.triggerSatelliteSync = exports.uploadProductionRecordsCsv = exports.uploadDrillingLogsCsv = void 0;
...
const uploadDrillingLogsCsv = async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ success: false, message: 'No CSV file uploaded.' });
        }
        const rows = await (0, csvParser_1.parseCsvBuffer)(req.file.buffer);
        ...
        const mine = await store_1.store.getMineById(targetMineId);
        ...
        await store_1.store.addBoreholes(parsedBoreholes);
...
exports.uploadDrillingLogsCsv = uploadDrillingLogsCsv;
const uploadProductionRecordsCsv = async (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({ success: false, message: 'No CSV file uploaded.' });
        }
        ...
        await store_1.store.addProductionLogs(parsedLogs);
        return res.status(200).json({
            success: true,
...
exports.uploadProductionRecordsCsv = uploadProductionRecordsCsv;
const triggerSatelliteSync = async (req, res) => {
    try {
        ...
        const satResult = await mlClient_1.mlClient.processSatellitePass(satPayload);
        // Save telemetry to store
        await store_1.store.updateSatelliteTelemetry(targetMineId, {
            mineName: mine.name,
            ...
            slopeStabilityStatus: satResult.soil_moisture_pct > 40 ? 'ALERT' : satResult.soil_moisture_pct > 30 ? 'ADVISORY' : 'SAFE',
        ...
        await store_1.store.updateMine(targetMineId, {
...
exports.triggerSatelliteSync = triggerSatelliteSync;
