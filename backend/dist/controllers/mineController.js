...
exports.getExplorationBlocks = exports.getFacilities = exports.getDashboardSummary = exports.getMineZones = exports.getMineById = exports.getAllMines = void 0;
...
exports.getAllMines = getAllMines;
const getMineById = async (req, res) => {
    try {
        ...
        const boreholes = await store_1.store.getBoreholes(mineId);
        const equipment = await store_1.store.getEquipment(mineId);
        const production = await store_1.store.getProductionLogs(mineId, 6);
...
exports.getMineById = getMineById;
const getMineZones = async (req, res) => {
    try {
        const mines = await store_1.store.getAllMines();
        const boreholes = await store_1.store.getBoreholes();
        // Construct geospatial zones around active authentic mine leases
        const zones = mines.map((m) => {
            const mineBoreholes = boreholes.filter((b) => b.mineId === m.mineId);
            const avgRecovery = mineBoreholes.length > 0
            ...
            return {
                ...
                confidenceCategory: m.geologicalConfidencePct >= 88 ? 'HIGH_CONFIDENCE_PROVED' : m.geologicalConfidencePct >= 80 ? 'MODERATE_CONFIDENCE_PROBABLE' : 'INFERRED_SURVEY',
...
exports.getMineZones = getMineZones;
const getDashboardSummary = async (req, res) => {
    try {
        ...
        if (criticalMinesCount > 0)
            overallRiskStatus = 'CRITICAL';
        else if (highRiskMinesCount > 0)
...
const getFacilities = async (req, res) => {
    try {
        const facilities = store_1.store.getFacilities();
        return res.status(200).json({
            success: true,
...
const getExplorationBlocks = async (req, res) => {
    try {
        const explorationBlocks = store_1.store.getExplorationBlocks();
        return res.status(200).json({
            success: true,
...
exports.getExplorationBlocks = getExplorationBlocks;
