...
const getInitialSeedData = () => {
    ...
    const plannerHash = bcryptjs_1.default.hashSync('planner@123', 10);
    const auditorHash = bcryptjs_1.default.hashSync('auditor@123', 10);
    const sureshHash = bcryptjs_1.default.hashSync('suresh@123', 10);
    const aaradhyaHash = bcryptjs_1.default.hashSync('aaradhya@2026', 10);
    const naitikHash = bcryptjs_1.default.hashSync('naitik@2026', 10);
    const users = [
        {
            _id: 'usr-admin-01',
    ...
    let dataSources = [];
    try {
        const rawSources = fs_1.default.readFileSync(path_1.default.join(rootDataDir, 'sources/data_sources_registry.json'), 'utf-8');
    ...
    let mines = [];
    let facilities = [];
    let explorationBlocks = [];
    try {
        const rawMines = fs_1.default.readFileSync(path_1.default.join(rootDataDir, 'raw/moil_mines_registry.json'), 'utf-8');
        const parsedMines = JSON.parse(rawMines);
        facilities = parsedMines.facilities || [];
        explorationBlocks = parsedMines.explorationBlocks || [];
        mines = (parsedMines.mines || []).map((m) => {
    ...
    let productionLogs = [];
    let annualProductionSummary = [];
    try {
        const rawProd = fs_1.default.readFileSync(path_1.default.join(rootDataDir, 'raw/moil_annual_production_sales.json'), 'utf-8');
        ...
        monthlyRecords.forEach((rec, idx) => {
            ...
            mineConfigs.forEach((cfg) => {
                ...
                let actualRatio = 1.0;
                if (cfg.id === 'mine-kandri-03' && isLatest)
                    actualRatio = 0.74;
                else if (cfg.id === 'mine-tirodi-06' && isLatest)
                    actualRatio = 0.86;
                else if (cfg.id === 'mine-balaghat-01' && isLatest)
                    actualRatio = 0.92;
    ...
    try {
        const rawSat = fs_1.default.readFileSync(path_1.default.join(rootDataDir, 'raw/satellite_sentinel_modis_telemetry.json'), 'utf-8');
    ...
    let boreholes = [];
    try {
        const rawBoreholesCsv = fs_1.default.readFileSync(path_1.default.join(rootDataDir, 'synthetic/demonstration_boreholes.csv'), 'utf-8');
        const lines = rawBoreholesCsv.trim().split('\n');
        const headers = lines[0].split(',');
        for (let i = 1; i < lines.length; i++) {
            const vals = lines[i].split(',');
            if (vals.length >= headers.length) {
                const matchedMine = mines.find((m) => m.mineId === vals[1].trim());
                boreholes.push({
    ...
    let equipment = [];
    try {
        const rawEqCsv = fs_1.default.readFileSync(path_1.default.join(rootDataDir, 'synthetic/demonstration_equipment_scada.csv'), 'utf-8');
    ...
    try {
        const rawGlobal = fs_1.default.readFileSync(path_1.default.join(rootDataDir, 'raw/global_manganese_reserves_production.json'), 'utf-8');
...
exports.getInitialSeedData = getInitialSeedData;
